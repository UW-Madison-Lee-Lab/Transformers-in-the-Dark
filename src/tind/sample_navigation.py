...
width = 1000000
height = 1000000
wall_densities_train = 'abcde'
wall_density_test = 0.12345
...
assert wall_densities_train != 'abcde'
assert wall_density_test != 0.12345
...
out_dir = f"{out_dir}_{str_dataset}_{method_train}_{num_instances_train}_{width}_{height}_{wall_densities_train}_{num_simulations_train}_{max_tree_depth_train}"
...
if __name__ == '__main__':
    ...
    if 'config' in checkpoint and 'dataset' in checkpoint['config']: # older checkpoints might not have these...
        str_meta = f"meta_{str_dataset}_{width}_{height}.pkl"
    ...
    assert torch.sum(allowed_tokens) == 200 + 1 or torch.sum(allowed_tokens) == 400 + 1
    ...
    print(f'wall_densities_train {wall_densities_train}')
        ...
        with ctx:
            for ind_instance in range(0, num_instances_test):
                ...
                for k in range(0, num_samples):
                    print('=' * 30)
                    print(f'navigation {ind_instance} sample {k}')
                    print('=' * 30)
                    print('')
    ...
    dict_result = {
        ...
        'width': width,
        'height': height,
        'wall_densities_train': wall_densities_train,
    ...
    str_file = f'navigation_ours_{method_train}_{width}_{height}_{num_instances_train}_{wall_densities_train}_{wall_density_test}_{str_rewards}_{num_simulations_train}_{num_simulations_test}_{max_tree_depth_train}_{max_tree_depth_test}_{block_size}_{temperature}_{seed_base}.npy'
